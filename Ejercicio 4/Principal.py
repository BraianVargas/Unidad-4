...
class Calculadora(object):
    ...
    def __init__(self):
        ...
        ttk.Button(mainframe, text='a/b', command=partial(self.ponerNUMERO, '/')).grid(column=1, row=2, sticky=W)
        ...
        if self.__panel.get().count("/") < 1:
            if self.__operadorAux==None:
                ...
                if self.__panel.get().count("/") == 1:
                    ...
                    valor=self.__panel.get()
                    self.__primerOperandoF1 = int(valor)
                    self.__segundoOperandoF2 = 1
                    self.__panel.set(numero)
        else:
            if(numero != '/'):
                if self.__operadorAux==None:
    ...
    def ponerOPERADOR(self, op):
        if op=='=':
            ...
            if (self.__panel.get().count("/") == 1):
                ...
                self.__primerOperandoF3 = int(self.__panel.get())
                self.__segundoOperadorF4=1
                self.resolverFraccion(self.__primerOperandoF1, self.__segundoOperandoF2, self.__primerOperandoF3, self.__segundoOperadorF4, operacion)
                self.__operador.set('')
                self.__operadorAux=None
        else:
